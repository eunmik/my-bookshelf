...
import static com.cleancode.chapter16.Month.FEBRUARY;
...
public class SpreadsheetDate extends DayDate {
    ...
    static final int[] AGGREGATE_DAYS_TO_END_OF_PRECEDING_MONTH =
            {0, 0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334, 3615};
    ...
    public SpreadsheetDate(int day, Month month, int year) {
        if(year < MINIMUM_YEAR_SUPPORTED || year > MAXIMUM_YEAR_SUPPORTED)
            throw new IllegalArgumentException("The 'year' argument must be in range" + MINIMUM_YEAR_SUPPORTED + " to " + MAXIMUM_YEAR_SUPPORTED + ".");
    ...
    private Month huntForMonthContaining(int anOriginal, int firstOrdinalOfYear) {
        int daysIntoThisYear = anOriginal - firstOrdinalOfYear;
        int aMonth = 1;
    ...
    public static DayDate createInstance(Date date){
        ...
        return new SpreadsheetDate(calendar.get(Calendar.DATE), Month.fromInt(calendar.get(Calendar.MONTH) + 1), calendar.get(Calendar.YEAR));
    }
